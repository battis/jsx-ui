...
export default class Navbar implements JSXComponent {
  public static NavbarBody = class implements JSXComponent {
    ...
    render(children?) {
      ...
      const root = document.querySelector(process.env.ROOT_SELECTOR || "#root");
      if (root) {
        new MutationObserver(
          this.resizeTopMarginWhenAttachedToDom.bind(this, this.element)
        ).observe(root, {
          attributes: false,
          childList: true,
          subtree: true,
      ...
      for (const mutation of mutations) {
        mutation.addedNodes.forEach((addedNode) => {
          if (addedNode.contains(element)) {
            observer.disconnect();
            const navbar = element.parentElement?.querySelector(".navbar");
            if (navbar) {
    ...
    resizeTopMarginWhenNavbarResizes(element, navbar) {
      element.style.marginTop = this.calculateMarginTop(navbar.clientHeight);
    }
  };
