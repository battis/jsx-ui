...
class TabContent extends Component {
    ...
    public constructor(props) {
        ...
        super({ ...rest });
        this._name = name;
        this.id = `tab-${Identifier.identifier()}`;
    }

    render(children?: Element[]) {
        return this.element = <div class='tab-content' id={this.id}>{children}</div>;
    }
}
...
export default class Tabbed extends Component {
    ...
    public select(tabId) {
        ...
        for (const tab of Array.from(this.element.querySelectorAll(`.tab-content:not([id="${tabId}"])`))) {
            tab.classList.remove('active');
            this.element.querySelector(`#label-for-${tab.id}`)?.classList.remove('active');
        }
    }
}
